...
public class SearchVoice {
	...
	public SearchVoice(String method, MainActivity mainActivity) {
		if (method.equals("resultSearch")) {
			if (mainActivity.getRequestCode() == 100) {
				if (mainActivity.getResultCode() == mainActivity.RESULT_OK
						&& mainActivity.getData() != null) {
					ArrayList<String> result = mainActivity.getData()
	...
	private void addSearchVoiceTablet(RelativeLayout rootView) {
		...
		imageView.setImageResource(Rconfig.getInstance().drawable(
				"ic_search_voice_tab"));
		imageView.setColorFilter(Config.getInstance().getTop_menu_icon_color());

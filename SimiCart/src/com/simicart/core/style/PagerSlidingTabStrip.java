...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	private int tabBackgroundResId = Rconfig.getInstance().drawable(
			"background_tab");
	...
	public PagerSlidingTabStrip(Context context, AttributeSet attrs,
		...
		int[] array_index = Rconfig.getInstance().getArrayStyleable(
				"PagerSlidingTabStrip");

		a = context.obtainStyledAttributes(attrs, array_index);

		indicatorColor = a.getColor(
				Rconfig.getInstance().getAttr(
						"PagerSlidingTabStrip_pstsIndicatorColor"),
				indicatorColor);
		underlineColor = a.getColor(
				Rconfig.getInstance().getAttr(
						"PagerSlidingTabStrip_pstsUnderlineColor"),
				underlineColor);
		dividerColor = a.getColor(
				Rconfig.getInstance().getAttr(
						"PagerSlidingTabStrip_pstsDividerColor"), dividerColor);
		indicatorHeight = a.getDimensionPixelSize(Rconfig.getInstance()
				.getAttr("PagerSlidingTabStrip_pstsIndicatorHeight"),
				indicatorHeight);
		underlineHeight = a.getDimensionPixelSize(Rconfig.getInstance()
				.getAttr("PagerSlidingTabStrip_pstsUnderlineHeight"),
				underlineHeight);
		dividerPadding = a.getDimensionPixelSize(
				Rconfig.getInstance().getAttr(
						"PagerSlidingTabStrip_pstsDividerPadding"),
				dividerPadding);
		tabPadding = a.getDimensionPixelSize(
				Rconfig.getInstance().getAttr(
						"PagerSlidingTabStrip_pstsTabPaddingLeftRight"),
				tabPadding);
		tabBackgroundResId = a.getResourceId(
				Rconfig.getInstance().getAttr(
						"PagerSlidingTabStrip_pstsTabBackground"),
				tabBackgroundResId);

...
public class ButtonRectangle extends Button {
	...
	protected void setDefaultProperties() {
		...
		// paddingLeft = Utils.dpToPx(12, getResources());
	...
	protected void setAttributes(AttributeSet attrs) {
		...
		if (text != null) {
			...
			int textColor = attrs.getAttributeResourceValue(ANDROIDXML,
					"textColor", -1);
			if (textColor != -1) {
				...
				// Color by hexadecimal
				// Color by hexadecimal
				textColor = attrs.getAttributeIntValue(ANDROIDXML, "textColor",
						-1);
				if (textColor != -1)
					textButton.setTextColor(textColor);
			}
			int[] array = { android.R.attr.textSize };
			TypedArray values = getContext().obtainStyledAttributes(attrs,
					array);

        ...
        while (true) {
            ...
            try {
                ...
                if (statusCode < 200 || statusCode > 299) {
                    SimiNetworkResponse networkResponse = new SimiNetworkResponse(statusCode, responseContents);
                    networkResponse.setStatusRequest(SimiNetworkResponse.StatusRequest.FAIL);
                ...
                SimiNetworkResponse networkResponse = new SimiNetworkResponse(SimiError.Error.SOCKET_TIMEOUT_ERROR, null);
                networkResponse.setStatusRequest(SimiNetworkResponse.StatusRequest.FAIL);
                return networkResponse;
            } catch (MalformedURLException e) {
                Log.e("SimiBasicNetwork " + request.getUrl(),
                        "MalformedURLException " + e.getMessage());
                SimiNetworkResponse networkResponse = new SimiNetworkResponse(SimiError.Error.MALFORMEDURL_ERROR, null);
                networkResponse.setStatusRequest(SimiNetworkResponse.StatusRequest.FAIL);
                return networkResponse;
            } catch (IOException e) {
                Log.e("SimiBasicNetwork " + request.getUrl(), "IOException "
                        + e.getMessage());
                SimiNetworkResponse networkResponse = new SimiNetworkResponse(SimiError.Error.IOEXCEPTION_ERROR, null);
                networkResponse.setStatusRequest(SimiNetworkResponse.StatusRequest.FAIL);
                return networkResponse;
            }
        }
